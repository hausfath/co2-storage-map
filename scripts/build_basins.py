...
def main():
    ...
    leftovers = []  # (geom, unit-record) for units outside every basin polygon
    for u in eu_fc["features"]:
        up = u["properties"]
        g = make_valid(shape(u["geometry"]))
        pt = g.representative_point()
        n_units += 1
        host = next((bg for bg, f in basin_shapes if bg.contains(pt)), None)
        rec = {"name": up.get("name"), "country": up.get("country"),
               "storage_type": up.get("storage_type") or "saline"}
        if host is not None:
            f = next(f for bg, f in basin_shapes if bg is host)
            f["properties"].setdefault("units", []).append(rec)
            n_attached += 1
        else:
            leftovers.append((g, rec))

    # Stacked stratigraphic units share one footprint (e.g. Muschelkalk over
    # Upper Carboniferous): cluster overlapping leftovers into one clickable
    # polygon that lists every stacked unit, instead of drawing them on top
    # of each other.
    parent = list(range(len(leftovers)))
    def find(i):
        while parent[i] != i:
    ...
    for i in range(len(leftovers)):
        for j in range(i + 1, len(leftovers)):
            a, b = leftovers[i][0], leftovers[j][0]
            if not a.intersects(b):
                continue
            ov = a.intersection(b).area
            if ov > 0.35 * min(a.area, b.area):
                parent[find(i)] = find(j)
    clusters = {}
    for i in range(len(leftovers)):
        clusters.setdefault(find(i), []).append(i)
    for members in clusters.values():
        geoms = [leftovers[i][0] for i in members]
        recs = [leftovers[i][1] for i in members]
        lead = recs[max(range(len(geoms)), key=lambda k: geoms[k].area)]
        name = lead["name"] if len(recs) == 1 else \
            f"{lead['name']} (+{len(recs) - 1} stacked units)"
        geom = unary_union(geoms).simplify(SIMPLIFY_DEG, preserve_topology=True)
        note = ("CO2StoP storage unit" if len(recs) == 1 else
                f"{len(recs)} CO2StoP storage units stacked at this location "
                f"(different formations/depths)")
        feats.append({"type": "Feature", "properties": {
            "basin": name, "countries": sorted({r["country"] for r in recs if r["country"]}),
            "cap_low_gt": None, "cap_mid_gt": None, "cap_high_gt": None,
            "tier": None, "soft_tier": False, "unit": True,
            "onshore_offshore": None,
            "src": "EU CO2StoP storage unit(s)",
            "notes": f"{note}; shown individually because no basin-level "
                     f"assessment polygon covers this area.",
            "units": recs,
            "_layer": "co2stop_unit"},
            "geometry": round_geom(mapping(geom))})
    print(f"EU CO2StoP: {n_attached}/{n_units} units attached to basins, "
          f"{len(leftovers)} standalone merged into {len(clusters)} polygons")
